...
export async function startWakeWordAudioMonitor(options: WakeWordAudioMonitorOptions): Promise<() => void> {
  ...
  const windowMillis = options.windowMillis ?? 7_200;
  const sliceMillis = options.sliceMillis ?? 1_500;
  const minSubmitIntervalMillis = options.minSubmitIntervalMillis ?? 11_000;
  const minSubmitBytes = options.minSubmitBytes ?? 18_000;
  const minRms = options.minRms ?? 0.028;
  const mimeType = resolveSupportedAudioMimeType();
  const stream = await navigator.mediaDevices.getUserMedia({
    audio: buildAudioConstraints(),
    video: false,
