...
export type LocalConversationResponse = {
  title: string;
  summary: string;
  detail: string;
  nextStep: string;
...
export function createCommandPlan(command: CommandInput): CommandExecutionPlan {
  const needsScreenCapture =
    command.contextMode === "screen" || command.contextMode === "auto";
  const needsProjectManifest =
    command.contextMode === "project" || command.contextMode === "auto";
...
export function resolveScreenTargetPolicy(
  command: CommandInput,
): ScreenTargetPolicy {
  const needsScreen =
    command.contextMode === "screen" || command.contextMode === "auto";
...
export function inferContextMode(
  ...
  if (isScreenIntent(intent)) {
    return intent === "screen_math_solver" && inlineMathMatched
      ? "general"
      : "screen";
  }
...
export function inferCommandIntent(
  text: string,
  hint: CommandRoutingHint = {},
): CommandIntent {
  const token = getCommandToken(text);
  ...
  if (token === "/screen")
    return inlineMathMatched ? "screen_math_solver" : "screen_error_analysis";
...
export function isWakePhraseText(text: string): boolean {
  ...
  if (/(헤이|해이|하이|에이|제이|hey|hi).{0,6}(자비스|자비|자비서|저비스|제비스|차비스|잡이스|서비스|자비스야)/.test(normalized)) {
    return true;
  }
  ...
  return wakeCandidates.some((candidate) => normalized.includes(candidate));
}
...
export function tryBuildLocalConversationResponse(
  text: string,
): LocalConversationResponse | null {
  ...
  if (isGreetingText(normalized)) {
    return {
      title: "대화 응답",
      summary: "안녕하세요. 듣고 있습니다. 편하게 말씀해주세요.",
      detail: "간단한 인사로 인식해서 바로 응답했습니다.",
      nextStep:
        "화면을 봐달라고 하거나, 계산·번역·프로젝트 분석을 자연스럽게 말씀해 주세요.",
    };
  }

  if (isThanksText(normalized)) {
    return {
      title: "대화 응답",
      summary: "좋습니다. 계속 듣고 있습니다.",
      detail: "감사 표현으로 인식해서 바로 응답했습니다.",
      nextStep: "다음에 필요한 작업을 편하게 말씀해 주세요.",
    };
  }
...
export function trySolveInlineMathCommand(
  text: string,
): InlineMathSolution | null {
...
function resolvePendingSummary(
  contextMode: ContextMode,
  intent: CommandIntent,
): string {
  if (contextMode === "screen") {
    return intent === "screen_math_solver"
      ? "Waiting for screen selection"
      : "Preparing screen context";
  }
...
function resolveNextStep(
  ...
  if (intent === "screen_error_analysis") {
    return contextMode === "auto"
      ? "Review screen and project diagnosis"
      : "Review screen diagnosis";
  }

  if (intent === "screen_math_solver") {
    return contextMode === "general"
      ? "Review the calculated result"
      : "Review the full solution in detail view";
  }
...
function inferSpokenScreenIntent(text: string): CommandIntent | null {
  ...
  if (/(번역|translate|translation)/.test(normalized))
    return "screen_translate";
  if (/(요약|정리|summary|summarize)/.test(normalized)) return "screen_summary";
  if (/(계산|수식|math|calculate|풀어)/.test(normalized))
    return "screen_math_solver";
  if (
    /(왜|오류|에러|문제|안됨|안돼|이상|고장|diagnos|error|issue|problem)/.test(
      normalized,
    )
  ) {
...
function hasScreenReference(text: string): boolean {
  return /(화면|스크린|창|현재 화면|보고 있는|캡처|캡쳐|screen|window|display)/.test(
    text,
  );
}
...
function hasInlineArithmeticExpression(text: string): boolean {
  return (
    ...
    trySolveInlineMathCommand(text) !== null
  );
}
...
function isGreetingText(text: string): boolean {
  ...
  if (/^(안녕|안녕하세요|하이|헬로|hello|hi|hey)$/.test(compact)) return true;
  if (/^(자비스안녕|자비스야안녕|헤이자비스안녕|하이자비스안녕)$/.test(compact))
    return true;
  return false;
}
...
function isThanksText(text: string): boolean {
  const compact = text.replace(/\s+/g, "");
  return /^(고마워|감사|감사합니다|땡큐|thanks|thankyou)$/.test(compact);
}
