...
Desktop에서 Local Agent 상태를 주기적으로 확인하고, 화면 OCR과 화면 분석을 Local Agent 우선으로 처리한다.
...
OCR 단계에서도 Backend / AI Server를 자동 fallback으로 사용하지 않는다. Local Agent OCR이 준비되지 않았으면 화면 명령은 안전하게 중단하고 사용자가 Local Assistant 설정을 확인하도록 안내한다.

## Security Policy

- Local Agent는 `127.0.0.1` loopback 주소만 사용한다.
- Desktop은 Local Agent를 `http://127.0.0.1:17997`로 호출한다.
- Local Agent가 unavailable이면 OCR/분석을 자동으로 원격 fallback하지 않는다.
- 전체 모니터 자동 캡처는 하지 않는다.
- 사용자가 선택한 화면/창 이미지는 Local Agent OCR로만 전달하며 NAS로 자동 전송하지 않는다.
...
3. Context Panel shows user-facing context status
4. User enters screen command
5. Desktop captures selected screen/window
6. Desktop sends capture to Local Agent OCR API
7. Local Agent validates image and extracts OCR text
8. Desktop receives OCR text
9. Desktop calls Local Agent analyze API
10. Local Agent routes the request to a model role
...
Desktop does not choose a model directly, and provider/model details are kept out of the user-facing command result UI.
