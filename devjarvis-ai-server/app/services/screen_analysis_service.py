...
def analyze_screen(request: ScreenAnalysisRequest) -> ScreenAnalysisResponse:
    ...
    text = _normalize_text(request.ocr_text)
    warnings: list[str] = []

    if not request.ocr_text_found or not text:
        warnings.append("OCR text was empty. Screen analysis is waiting for readable text.")
    ...
    return ScreenAnalysisResponse(
        ...
        provider="screen-analysis-placeholder",
...
def _build_placeholder_result(intent: str, text: str) -> tuple[str, str, str, list[str]]:
    if intent == "screen_translate":
        return (
            ...
            _build_detail("Translation provider is not connected yet. The OCR text below is the sanitized input that will be passed to the next LLM or translation provider.", text),
            [
    ...
    if intent == "screen_summary":
        return (
            ...
            _build_detail("Summarization provider is not connected yet. The OCR text below is the sanitized input that will be summarized in the next stage.", text),
            [
    ...
    if intent == "screen_error_analysis":
        return (
            ...
            _build_detail("Diagnosis provider is not connected yet. The OCR text below is the sanitized input that will be combined with project context or RAG later.", text),
            [
    ...
    return (
        ...
        _build_detail("A specialized analysis provider is not connected yet. The OCR text below is the sanitized input for the next stage.", text),
        [
