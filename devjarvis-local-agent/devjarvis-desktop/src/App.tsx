...
const WAKE_AUDIO_WINDOW_MILLIS = 5800;
const WAKE_AUDIO_SLICE_MILLIS = 1000;
const WAKE_AUDIO_SUBMIT_INTERVAL_MILLIS = 4500;
const WAKE_AUDIO_MIN_SUBMIT_BYTES = 9000;
const WAKE_AUDIO_MIN_RMS = 0.012;
...
function App() {
  ...
  useEffect(() => {
    ...
    async function startMonitor() {
      if (cancelled || micAvailable !== true || !localSttReady) {
        return;
      }
        ...
        stopMonitor = await startWakeWordAudioMonitor({
          ...
          onStateChange: (state) => {
            ...
            if (state === "listening" && voiceStateRef.current === "idle") {
              setWakeStatusMessage("헤이 자비스 대기 · 마이크 감지 중");
              setWakeDiagnosticMessage("음성이 들어오면 약 4~5초 간격으로 호출어를 확인합니다.");
            }
          },
          ...
          onAudio: async (audio, diagnostics) => {
            ...
            try {
              ...
              if (wakeMatched) {
                ...
                if (!cancelled && voiceStateRef.current === "idle") {
                  setWakeStatusMessage("헤이 자비스 대기 · 마이크 감지 중");
                }
                return;
              }
