...
export function tryBuildLocalConversationResponse(
  ...
  if (isGreetingText(normalized)) {
    return {
      title: "인사 응답",
      summary: "안녕하세요. 준비되어 있습니다. 무엇을 도와드릴까요?",
      detail: "사용자의 인사에 로컬에서 바로 응답했습니다.",
      nextStep:
        "화면 분석, 프로젝트 분석, 계산, 번역처럼 필요한 작업을 말씀해 주세요.",
    };
  }
  ...
  if (isThanksText(normalized)) {
    return {
      title: "응답 완료",
      summary: "언제든요. 필요한 게 있으면 바로 말씀해 주세요.",
      detail: "감사 표현에 로컬에서 바로 응답했습니다.",
      nextStep: "다음 작업을 말씀해 주세요.",
    };
  }
