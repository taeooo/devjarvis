...
class LocalSttService:
    ...
    async def transcribe(
        ...
        *,
        ...
        if self.settings.stt_provider == "faster_whisper":
            return await self._transcribe_with_faster_whisper(audio_bytes, filename, purpose=purpose or 'command')

        return LocalSttTranscribeResponse(status="unavailable", warnings=["local_stt_provider_unavailable"])

    async def _transcribe_with_faster_whisper(self, audio_bytes: bytes, filename: str, *, purpose: str = 'command') -> LocalSttTranscribeResponse:
        try:
        ...
        try:
            ...
            transcribe_kwargs = {
                ...
                "initial_prompt": resolve_initial_prompt(self.settings.stt_initial_prompt, is_wake),
            }
            if is_wake:
            ...
            segments, _info = model.transcribe(str(temp_path), **transcribe_kwargs)
...
def resolve_initial_prompt(base_prompt: str, is_wake: bool) -> str:
    if not is_wake:
        return base_prompt
    return "헤이 자비스. 하이 자비스. 해이 자비스. 에이 자비스. 제이 자비스. 자비스야. 자비스."
